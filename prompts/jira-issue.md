...
ASSIGNEE="$(jira me)"
issue_key=$(jira issue create -tTask @JIRA_LABEL_ARGS@ -s "$TITLE" -b "$DESCRIPTION" --custom Responsible-Squad="@JIRA_RESPONSIBLE_SQUAD@" --custom Acceptance-criteria="$ACCEPTANCE_CRITERIA" --no-input --raw | jq -r '.key')
