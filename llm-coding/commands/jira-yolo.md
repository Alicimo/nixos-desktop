...
Delegate a final review to the `code-reviewer` agent. Supply it with:
...
Fix every Critical and Important finding and rerun affected checks. After all fixes and generated files are complete, require one final `code-reviewer` pass over the exact final worktree diff, including untracked files. Do not commit with unresolved Critical or Important findings or with changes added after the final review.
